...
const programs = [
  {
    ...
    description:
      "Our education initiatives provide completely free schooling to children from disadvantaged backgrounds. We believe education is the foundation of progress and the key to breaking the cycle of poverty.",
    features: [
  ...
  },
  {
    ...
    description:
      "Food insecurity affects millions of families struggling with financial hardships. Through our Monthly Ration Distribution Program, we ensure that essential food supplies reach those who need them most.",
    features: [
  ...
  },
  {
    ...
    description:
      "Access to quality healthcare is a fundamental right. Our healthcare programs provide medical aid, health awareness, and support to underserved communities who cannot afford essential medical services.",
    features: [
      "Free medical camps",
      "Health screenings",
  ...
  },
  {
    ...
    description:
      "In today's digital world, access to technology and digital skills is essential. We provide underprivileged children with digital education, equipping them with tools to learn, grow, and succeed.",
    features: [
  ...
  },
  {
    ...
    description:
      "We stand in solidarity with those facing crises worldwide. Our global aid programs support displaced and struggling families, providing emergency relief and long-term assistance.",
    features: [
  ...
  },
  {
    ...
    description:
      "When disasters strike, we respond quickly and effectively. Our emergency relief programs provide immediate assistance to affected communities, helping them recover and rebuild.",
    features: [
...
const impactStats = [
  { icon: Users, value: "15,000+", label: "Lives Impacted", color: "#103B40" },
  { icon: Globe, value: "50+", label: "Countries", color: "#246B73" },
  { icon: Shield, value: "100%", label: "Transparency", color: "#103B40" },
  {
    icon: CheckCircle,
    value: "500+",
  ...
  return (
    <div ref={sectionRef}>
      <section className="pt-32 pb-20 bg-[#F9F5F3] relative overflow-hidden">
        ...
        <div className="absolute top-1/2 right-0 w-96 h-96 bg-[#246B73]/10 rounded-full blur-3xl" />
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <span className="animate-fade-up animate-in opacity-0 inline-flex items-center gap-2 glass-panel px-4 py-2 rounded-full mb-6">
            ...
            <span className="text-[#1a2e2f] text-sm font-medium">Our Work</span>
          </span>
      ...
      <section className="py-20 md:py-32 bg-[#F9F5F3]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative">
            <div className="absolute left-1/2 transform -translate-x-px h-full w-0.5 bg-gradient-to-b from-[#103B40] to-[#246B73] hidden lg:block" />
            <div className="space-y-24">
              {programs.map((program, index) => (
                <div
                  key={program.id}
                  id={program.id}
                  className={`animate-fade-up animate-in opacity-0 relative grid lg:grid-cols-2 gap-12 items-center ${
                    index % 2 === 1 ? "lg:flex-row-reverse" : ""
                  }`}
                >
                  <div className={index % 2 === 1 ? "lg:order-2" : ""}>
                    <div
                      className={`inline-flex items-center gap-2 bg-gradient-to-r ${program.color} bg-clip-text`}
                    >
                      <div
                        className={`w-14 h-14 rounded-xl bg-gradient-to-br ${program.color} flex items-center justify-center`}
                      >
                        <program.icon className="w-7 h-7 text-white" />
                      </div>
                    </div>
                    <h3 className="text-2xl md:text-3xl font-display font-semibold text-[#1a2e2f] mt-6 mb-2">
                      {program.title}
                    </h3>
                    <p
                      className={`text-lg font-medium mb-4 bg-gradient-to-r ${program.color} bg-clip-text text-transparent`}
                    >
                      {program.tagline}
                    </p>
                    <p className="text-[#4a6266] leading-relaxed mb-6">
                      {program.description}
                    </p>
                    <div className="flex items-center gap-2 font-medium">
                      <span style={{ color: "#103B40" }}>Impact:</span>
                      <span className="text-[#1a2e2f]">{program.impact}</span>
                    </div>
                  </div>

                  <div
                    className={`glass-card rounded-2xl p-8 ${index % 2 === 1 ? "lg:order-1" : ""}`}
                  >
                    <h4 className="text-lg font-display font-semibold text-[#1a2e2f] mb-6">
                      What We Offer:
                    </h4>
                    <ul className="space-y-4">
                      {program.features.map((feature, i) => (
                        <li key={i} className="flex items-start gap-3">
                          <CheckCircle className="w-5 h-5 text-[#103B40] flex-shrink-0 mt-0.5" />
                          <span className="text-[#4a6266]">{feature}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                  <div className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 w-5 h-5 rounded-full bg-gradient-to-br from-[#103B40] to-[#246B73] border-4 border-[#F9F5F3] shadow-lg z-10 hidden lg:block" />
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            {impactStats.map((stat, index) => (
              ...
              >
                ...
                <div className={`text-[${stat.color}]/70`}>{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 md:py-32 relative overflow-hidden bg-[#246B73]">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="animate-fade-up animate-in opacity-0 text-white/70 text-lg mb-10 max-w-2xl mx-auto">
            Your support enables us to continue our vital work. Every donation,
            no matter the size, helps us reach more people in need.
          </p>
